 ...
 * @category   	Core
 * @package    	FTP
 * @description	FTP Layer for write data on web space
 * @version    	0.1.0	
 * @date			dom 20 feb 2011 19:02:47 CET 
 * @author		Louis Landry  <louis.landry@joomla.org>
 * @edit by		Daniele Contarino
 * @contact edit	http://www.danielecontarino.it
 *
 */
...
if (!extension_loaded('ftp')) {
    if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
        ini_set('extension', 'php_ftp.dll');
        //@ dl('php_ftp.dll');
    } else {
        ini_set('extension', 'ftp.so');
        //@ dl('ftp.so');
    }
}
...
class FTP{
    ...
    /**
     * Method to determine if is logged to FTP server
     *
     * @access	public
    ...
    function listDetails($path = null, $type = 'all') {
        ...
        if ($osType == 'UNIX') {
            foreach ($contents as $file) {
                $tmp_array = null;
                if (preg_match_all($regexp, $file, $regs)) {
                    $fType = (int) strpos("-dl", $regs[1][0][0]);
                    //$tmp_array['line'] = $regs[0][0];
                    $tmp_array['type'] = $fType;
                    $tmp_array['rights'] = $regs[1][0];
                    //$tmp_array['number'] = $regs[2][0];
                    $tmp_array['user'] = $regs[3][0];
                    $tmp_array['group'] = $regs[4][0];
                    $tmp_array['size'] = $regs[5][0];
                    $tmp_array['date'] = date("m-d", strtotime($regs[6][0]));
                    $tmp_array['time'] = $regs[7][0];
                    $tmp_array['name'] = $regs[9][0];
                }
                // If we just want files, do not add a folder
                if ($type == 'files' && !empty($tmp_array) && $tmp_array['type'] == 1) {
                    continue;
                }
                // If we just want folders, do not add a file
                if ($type == 'folders' && !empty($tmp_array) && $tmp_array['type'] == 0) {
                    continue;
                }
        ...
        elseif ($osType == 'MAC') {
            ...
            foreach ($contents as $file) {
                $tmp_array = null;
                if (ereg($regexp, $file, $regs)) {
                    $fType = (int) ($regs[7][0] == '<DIR>');
                    $timestamp = strtotime("$regs[3][0]-$regs[1][0]-$regs[2][0] $regs[4][0]:$regs[5][0]$regs[6][0]");
                    //$tmp_array['line'] = $regs[0][0];
                    $tmp_array['type'] = $fType;
                    $tmp_array['rights'] = '';
                    //$tmp_array['number'] = 0;
